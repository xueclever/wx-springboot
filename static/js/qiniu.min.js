    ...
    function QiniuJsSDK() {
        ...
        this.resetUploadUrl = function () {
            ...
            var e = function (b) {
                ...
                var h;
                g && 9 >= g ? (h = new mOxie.XMLHttpRequest, mOxie.Env.swf_url = a.flash_swf_url) : h = that.createAjax(), h.open("GET", f, !1);
                var i = function () {
            ...
            var g = function (b) {
                if (a.uptoken) that.token = a.uptoken; else if (a.uptoken_url) {
                    ...
                    if (c.open("GET", that.uptoken_url, !1), c.setRequestHeader("If-Modified-Since", "0"), c.send(), 200 === c.status) {
                        ...
                        var j = h(new Date);
                        that.tokenInfo = {
                            serverDelay: j - i, deadline: g.deadline, isExpired: function () {
            ...
            logger.debug("new plupload.Uploader(option)"), p.bind("Init", function (b, c) {
                ...
                if (i = i || b.settings && b.settings.chunk_size, logger.debug("uploader.runtime: ", p.runtime), logger.debug("chunk_size: ", i), "html5" !== p.runtime && "flash" !== p.runtime || !i) logger.debug("directUpload because uploader.runtime !== 'html5' || uploader.runtime !== 'flash' || !chunk_size"), d(b, c, that.key_handler); else if (c.size < i || e()) logger.debug("directUpload because file.size < chunk_size || is_android_weixin_or_qq()"), d(b, c, that.key_handler); else {
                    ...
                    s && 9 >= s && (r.accept = "text/plain; charset=utf-8", logger.debug("add accept text/plain in multipart params")), b.setOption({
                        ...
                        headers: {Authorization: "UpToken " + f(c)},
                        multipart_params: r
                    })
                }
                ...
                g = g || a.settings && a.settings.chunk_size, g > e && (a.setOption({url: qiniuUploadUrl + "/mkblk/" + e}), logger.debug("up.setOption url: ", qiniuUploadUrl + "/mkblk/" + e)), a.setOption({headers: {Authorization: "UpToken " + f(b)}}), localStorage.setItem(b.name, that.stringifyJSON({
                    ctx: l,
                    percent: b.percent,
                    total: c.total,
                    offset: c.offset,
